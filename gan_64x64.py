...
BATCH_SIZE = 16#32 # Batch size. Must be a multiple of N_GPUS
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    all_real_data_conv = tf.placeholder(tf.int32, shape=[BATCH_SIZE, 3, IMG_SIZE, IMG_SIZE])
    ...
    for device_index, (device, real_data_conv) in enumerate(zip(DEVICES, split_real_data_conv)):
        with tf.device(device):
            ...
            # print(real_data.get_shape(), fake_data.get_shape(), BATCH_SIZE//len(DEVICES))
